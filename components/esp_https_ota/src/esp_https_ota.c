...
// Define this symbol to enable the check used to detect
// when the HTTP file download has finished.
#define HTTP_FILE_DOWNLOAD_FINISHED_CHECK

// Define this symbol to enable the OTA terminate hack
#define OTA_TERMINATE_HACK
...
#ifdef HTTP_FILE_DOWNLOAD_FINISHED_CHECK
    http_event_handle_cb httpEvtHandler;
#endif
...
#ifdef OTA_TERMINATE_HACK
// Stashed OTA handle used by esp_https_ota_terminate()
static esp_https_ota_handle_t httpsOtaHandle = NULL;
static bool otaUpdateTerminated = false;
#endif
...
esp_err_t esp_https_ota_begin(const esp_https_ota_config_t *ota_config, esp_https_ota_handle_t *handle)
{
...
#ifdef OTA_TERMINATE_HACK
    // We need to stash the OTA handle so that the
    // app can abort the update when needed, by
    // calling: esp_https_ota_terminate();
    httpsOtaHandle = https_ota_handle;
    otaUpdateTerminated = false;
#endif

#ifdef HTTP_FILE_DOWNLOAD_FINISHED_CHECK
    // We need to stash the HTTP callback handler so
    // that we can post the dummy ON_DATA event to
    // inform the caller when the file download is
    // done.
    https_ota_handle->httpEvtHandler = ota_config->http_config->event_handler;
#endif
    ...
    if (err != ESP_OK) {
#ifdef OTA_TERMINATE_HACK
        if (otaUpdateTerminated) {
            // This error is expected when the user
            // asked to terminate the OTA update...
        } else
#endif
...
#ifdef HTTP_FILE_DOWNLOAD_FINISHED_CHECK
            } else if (data_read == -ESP_ERR_HTTP_EAGAIN) {
                ESP_LOGD(TAG, "ESP_ERR_HTTP_EAGAIN invoked: Call timed out before data was ready");
                // If we have already received all the data, it means the
                // download is finished, and therefore we can proceed with
                // the OTA update, which we indicate by setting the state
                // to ESP_HTTPS_OTA_SUCCESS. And to let the caller know that
                // the file download is done, we post a dummy ON_DATA event
                // with a null data pointer a zero data length.
...
#ifdef OTA_TERMINATE_HACK
// The app which started the OTA update now wants to
// abort it, typically because it is already running
// the latest version of the firmware....
esp_err_t esp_https_ota_terminate(void)
{
    esp_https_ota_t *handle = (esp_https_ota_t *) httpsOtaHandle;
    esp_err_t err;
    ...
    if ((err = esp_http_client_close(handle->http_client)) == ESP_OK) {
        // We use this flag to squelch some harmless
        // error conditions when we terminate an OTA
        // update...
        otaUpdateTerminated = true;
    }
